...
1. 由于业务逻辑散步在多个服务上，因此在微服务架构中开发复杂的业务逻辑更具挑战性， 这个过程需要解决两个关键问题：
   * 典型的领域模型是各种类（Class）交织在⼀起的⼀个⽹络，虽然在单体应⽤程序中 这不是问题，但在微服务架构中，类分散在不同的服务中，需要避免跨越服务边界 （进程）的对象引⽤。
   * 设计在微服务下的业务逻辑会受到微服务下事务管理的种种约束。单个服务内部可以 使⽤ACID事务，跨服务间的业务逻辑必须使⽤Saga模式来维护服务之间的数据⼀致 性。
2. 可以使⽤领域驱动设计中的聚合模式（Aggregate）来解决这些问题。聚合模式下，服务 的业务逻辑通过多个聚合组成⼀个集合来体现。聚合是⼀组对象，可以作为⼀个单元来处 理。在微服务架构中开发业务逻辑时，聚合可以起到以下两个重要的作⽤：
   * 使⽤聚合可以避免任何跨服务边界的对象引⽤，因为聚合之间通过主键进⾏引⽤，⽽ 不是通过对象的地址进⾏引⽤。
   * 由于单个事务只能创建或更新单个聚合，因此聚合满⾜微服务事务模型的约束。
3. 组织业务逻辑由两种主要模式：⾯向过程的事务脚本模式和⾯向对象的领域建模模式。
   1.  业务逻辑组织模式------ 在开发简单的业务逻辑时，使⽤⾯向对象的设计⽅法会有⼀种”杀鸡⽤⽜⼑“的感觉， 这种情况下，更好的⽅法是编写⾯向过程的代码，并使⽤Martin Fowler在《Patterns of Enterprise Application Architecture》⼀书中提到的事务脚本模式（将业务逻辑组 织为⾯向过程的事务脚本的集合，每种类型的请求都有⼀个脚本）
   2. 使⽤领域驱动模型模式设计业务逻辑 ------ *（将业务逻辑组织为具有状态和⾏为的类构成的对象模型）*在⾯向对象的设计中，业务逻辑由对象模型和相对较⼩的⼀些类的⽹络组成。这些类 通常直接对应于问题域中的概念。在这样的设计中，有些类只有状态或⾏为，但很多 类同时包含状态和⾏为，这样的类都是精⼼设计的
   3.   使⽤聚合模式设计领域模型------ 聚合是⼀个边界内的领域对象的集群，可以将其视为⼀个单元。它由根实体和可能的 ⼀个或多个其他实体和值对象组成。许多业务对象都被建模为聚合

## 六、使用时间溯源开发业务逻辑

1. 事件溯源开发业务逻辑概述------事件溯源，将聚合以⼀系列事件的⽅式持久化保存，每个事件代表聚合的⼀次状态变化， 应⽤程序通过重放（Replaying）事件来重新创建聚合的当前状态。

2. 事件溯源是⼀种以事件为中⼼的技术，⽤于实现业务逻辑和聚合的持久化。聚合作为⼀系 列事件存储在数据库中。每个事件代表聚合的状态变化。聚合的业务逻辑围绕⽣成和使⽤ 这些事件的要求⽽构建。让我们看看它是如何⼯作的。

   1. 事件溯源通过事件来持久化聚合

   ![image-20230520115011293](./6-0.png)

   当应⽤程序创建或更新聚合时，它会将聚合发出的事件插⼊到EVENTS表中。应⽤程 序通过从事件存储中检索并重放事件来加载聚合。加载聚合包含以下三个步骤： 加载聚合的事件 使⽤其默认构造函数创建聚合实例 调⽤apply()⽅法遍历事件。

   2. 事件代表状态的变化

      使⽤事件溯源时，事件不再是可有可⽆的。每当聚合的状态发⽣变化时，它必须发出 ⼀个事件。例如，Order聚合必须在创建时发出OrderCreated事件，并在更新时发出 Order*事件。这是⼀个⽐以前更严格的要求，在此之前，聚合只需要发出那些外部接 收⽅感兴趣的事件。

   3. 聚合⽅法都和事件有关

      业务逻辑通过调⽤聚合根上的命令⽅法（command method)来处理对聚合的更新请 求。在传统的应⽤程序中，命令⽅法通常会验证其参数，然后更新⼀个或多个聚合字 段。基于事件溯源的应⽤程序中的命令⽅法则通过⽣成事件来起作⽤

3. 使⽤乐观锁处理并发更新

4. 使⽤快照提升性能

   在⼀个Order聚合的⽣命周期中往往不会发⽣很多次状态转换，因此它只有少量事件。查 询事件存储库，找到这些事件并重新构建Order聚合是可⾏的。但是，⻓⽣命周期的聚合 可能会有⼤量事件，例如：Account聚合可能包含⼤量事件，随着事件的推移，加载和重 放这些事件会变得越来越低效。 常⻅的解决⽅案就是定期持久化保存聚合状态的快照，应⽤程序通过加载最新快照以及加 载快照后发⽣的事件来快速恢复聚合状态

   ![image-20230520115444585](./6-1.png)

5. 消息的幂等处理

   如果可以使⽤相同的消息多次安全的调⽤消息接收⽅，则消息接收⽅式幂等的。例如，通 过检测和丢弃重复消息来实现幂等的消息处理。 

   * 关系型数据库事件存储库的幂等消息处理 

     如果应⽤程序使⽤基于关系型数据库的事件存储库，则它可以使⽤相同的⽅法来检测 和丢弃重复消息。它将消息ID插⼊PROCESSED MESSAGES表，作为插⼊EVENTS 表的事件的事务的⼀部分。

   * NOSQL数据库事件存储库的幂等消息处理 

     消息消费者将消息的ID存储在处理它时⽣成的事件中，通过验证聚合的所有事件中是 否包含该消息ID来进⾏重复检测。

6. 事件溯源的优势
   * 可靠地发布领域事件。 
   * 保留聚合的历史。 
   * 最⼤限度地避免对象与关系的“阻抗失调”问题。 
   * 为开发者提供⼀个“时光机”。
7. 事件溯源的弊端 
   * 这类编程模式有⼀定的学习曲线。
   *  基于消息传递的应⽤程序的复杂性。
...
1. 自动化测试通常包括四个阶段
   ...
   4. 清理环境:必要时清理测试环境。许多测试省略了这个阶段，但是某些类型的测试比如涉及数据库的测试可能需要在这个阶段将数据库的状态回滚到设置环境阶段前的初始状态。

2. 测试替身(Test double)来消除被测系统的依赖性。测试替身是一个对象，该对象负责模拟依赖项的行为。有两种类型的测试替身:(stub)和模(mock)。术语和模拟通常可以互换使用,尽管它们的行为略有不同。桩是一个测试替身，它代替依赖项来向被测系统发送调用的返回值。模拟是也一个测试替身，用来验证被测系统是否正确调用了依赖项。此外，模拟通常也扮演桩的角色，向被测系统发送调用的返回值。

3. 组织测试的第一种方法---测试象限

   ![image-20230519095234624](./9-0.png)

4. 组织测试的方法--测试金字塔，它提供了关于每种测试类型要编写测试数量的指南

   ![image-20230519095627773](./9-1.png)

5. 消费者契约测试侧重于验证提供者 API的参数定义是否符合消费者的期望。对于REST接口，契约测试将验证提供者程序实现的接口是否:
   ...
   * 返回预期中的响应，包括状态代码、头部和主体。
     ...
     **重要的是要记住，契约测试不会彻底测试提供者的业务逻辑。这是单元测试的工作。稍你将看到RESTAPI的消费者契约测试实际上是通过模拟控制器进行的测试。**
...
6. 单元测试
   ...
   * 协作型单元测试:测试一个类及其依赖项
     ...
     ![image-20230526162110185](./9-2.png)



## 十、微服务架构中的测试策略（下）

1. 使用契约作为示例消息来驱动服务之间交瓦的测试。编写测试以验证两个服务的适配器是否符合契约，而不是编写运行两个服务及其传递依赖关系的慢速测试
2. 编写组件测试以通过其API验证服务的行为。应该通过单独测试服务来简化和加速组件测试，使用桩来解决其依赖项。
3. 编写用户旅程测试，以最大限度地减少端到端测试的数量，这些测试缓慢、脆弱又耗时。用户旅程测试模拟用户在应用程序中的旅程，并验证相对较大的应用程序功能片段的高级行为。因为测试很少，所以每次测试开销的数量(例如测试设置)被最小化这就加快了测试速度。
